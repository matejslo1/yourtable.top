...
import { sendConfirmationEmail, sendCancellationEmail } from './notificationService.js';
...
export async function createReservation(params: CreateReservationParams) {
  ...
  if (status === 'CONFIRMED') {
    sendConfirmationEmail(reservation.id).catch(err =>
...
/**
 * Update reservation status with validation and notifications
 */
export async function updateReservationStatus(
  ...
  if (newStatus === 'CONFIRMED') {
    sendConfirmationEmail(reservationId).catch(err =>
      console.error('[Notification] Confirmation email failed:', err)
    );
  } else if (newStatus === 'CANCELLED') {
    sendCancellationEmail(reservationId, reason).catch(err =>
      console.error('[Notification] Cancellation email failed:', err)
    );
  }
...
export async function updateReservation(
  ...
  if (data.tableIds) {
    ...
    const currentTables = await prisma.reservationTable.findMany({ where: { reservationId } });
    for (const ct of currentTables) { occupiedIds.delete(ct.tableId); }
