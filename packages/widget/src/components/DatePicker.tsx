  ...
  return (
    <div className="yt-select-none">
      ...
      <div className="yt-flex yt-items-center yt-justify-between yt-mb-4">
        <button
          onClick={prevMonth}
          className="yt-w-9 yt-h-9 yt-flex yt-items-center yt-justify-center yt-rounded-full hover:yt-bg-surface-100 yt-transition-colors yt-text-surface-500"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 12L6 8L10 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </button>
        <span className="yt-font-display yt-font-semibold yt-text-surface-900">
      ...
      <div className="yt-grid yt-grid-cols-7 yt-mb-1">
        {DAYS_SL.map(d => (
          <div key={d} className="yt-text-center yt-text-xs yt-font-medium yt-text-surface-400 yt-py-1">
            {d}
          </div>
        ))}
      </div>
        ...
        {days.map(({ date, inMonth, disabled }, i) => (
          <button
            ...
            className={`
              ...
              ${!inMonth ? 'yt-text-surface-200' : ''}
              ${disabled && inMonth ? 'yt-text-surface-300 yt-cursor-not-allowed' : ''}
              ${!disabled && inMonth && !isSelected(date) ? 'yt-text-surface-700 hover:yt-bg-brand-50 hover:yt-text-brand-700 yt-cursor-pointer' : ''}
              ${isSelected(date) ? 'yt-bg-brand-600 yt-text-white yt-font-semibold yt-shadow-sm' : ''}
              ${isToday(date) && !isSelected(date) ? 'yt-ring-1 yt-ring-brand-300 yt-font-semibold' : ''}
            `}
          >
            {date.getDate()}
          </button>
